// Headless smoke test for Q1 Ariakit + preact/compat spike.
// Walks Dialog / Tabs / Popover scenarios, captures console.error + pageerror,
...
try {
  ...
  log(`[smoke] Dialog after ESC: focus is on ${focusedAfterEsc}`);

  log('[smoke] Dialog: re-open and overlay click');
  await page.click('[data-testid="dialog-trigger"]');
  await page.waitForSelector('[data-testid="dialog-panel"]', { state: 'visible' });
  // Click overlay in top-left of viewport (outside dialog panel).
  await page.mouse.click(5, 5);
  await page.waitForTimeout(300);
  const stillOpen = await page.$('[data-testid="dialog-panel"]:visible');
  ...
  const activeTab = await page.getAttribute(
  ...
  await page.mouse.click(5, 5);
  await page.waitForTimeout(300);
  const popStillOpen = await page.$('[data-testid="popover-panel"]:visible');
  ...
  await fs.writeFile(errorFile, JSON.stringify({ consoleErrors, pageErrors }, null, 2));
