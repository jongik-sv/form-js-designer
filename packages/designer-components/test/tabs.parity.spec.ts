/**
 * TSK-04-02: Tabs parity spec — viewer↔editor 픽셀 diff ≤ 0.1%, 3 viewport
 *
...
test.describe('Tabs parity (viewer ↔ editor)', () => {
  ...
  test('tabs DOM exists in viewer', async ({ page }) => {
    await expect(page.locator('#viewer-root [data-component="tabs"]')).toBeVisible();
  });

  test('tabs DOM exists in editor', async ({ page }) => {
    await expect(page.locator('#editor-root [data-component="tabs"]')).toBeVisible();
  });

  test('tabs trigger labels are visible', async ({ page }) => {
