/**
 * Stack parity spec — viewer↔editor 픽셀 diff ≤ 0.1%, 3 viewport
 * QA 체크리스트: Stack parity (1024/1440/1920)
 *
...
test.describe('Stack parity (viewer ↔ editor)', () => {
  ...
  test('stack DOM exists in viewer', async ({ page }) => {
    await expect(page.locator('#viewer-root [data-component="stack"]')).toBeVisible();
  });

  test('stack DOM exists in editor', async ({ page }) => {
    await expect(page.locator('#editor-root [data-component="stack"]')).toBeVisible();
  });
});
