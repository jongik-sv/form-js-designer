/**
 * height-roundtrip.test.ts — TSK-12-04 CLI 단위 테스트
 *
 * QA 체크리스트:
 * - height-roundtrip.schema.json fixture가 runValidate() → exit 0
 * - fixture parse 후 components[0].layout.height === 200 / components[1].layout.rowHeight === 150
...
describe('height-roundtrip fixture', () => {
  ...
  it('components[1].layout.rowHeight === 150', () => {
    const raw = fs.readFileSync(FIXTURE_PATH, 'utf-8');
    const schema = JSON.parse(raw) as {
      components: Array<{ layout?: { rowHeight?: number } }>;
    };
